...
class RouteNavigationApp {
        ...
        try {
            ...
            if (!validation.valid) {
                if (validation.expired) {
                    this.showError('Link Expirado', 
                        'Este QR Code já expirou. Solicite um novo QR Code no porteiro.');
                } else {
            ...
            this.showError('Erro de Processamento', 
                'Não foi possível processar os dados da rota. Verifique o QR Code.');
        }
    }
    ...
    addMarkers() {
        ...
        const startMarker = new atlas.data.Feature(new atlas.data.Point([
        ...
        const endMarker = new atlas.data.Feature(new atlas.data.Point([
    ...
    async calculateRoute() {
        ...
        try {
            ...
            // Preparar requisição para a API de rotas
            const routeUrl = `${AZURE_MAPS_CONFIG.routeServiceUrl}?api-version=1.0&subscription-key=${AZURE_MAPS_CONFIG.subscriptionKey}`;
            ...
            if (!response.ok) {
                throw new Error(`Erro na API de rotas: ${response.status}`);
            }
    ...
    createDirectRoute() {
        ...
        document.getElementById('step-by-step').innerHTML = `
            <div class="navigation-instruction">
                <div class="icon">📍</div>
                <div class="text">
                    <strong>Direção direta</strong><br>
                    Siga em direção ao destino: ${this.doca.nome}
                </div>
                <div class="distance">${DataFormatter.formatDistance(distance)}</div>
            </div>
        `;
    ...
    generateInstructions(route) {
        ...
        if (route.guidance && route.guidance.instructions) {
            route.guidance.instructions.forEach((instruction, index) => {
                ...
                instructionsHTML += `
                    <div class="navigation-instruction" data-step="${index}">
                        <div class="icon">${icon}</div>
                        <div class="text">
                            <strong>${instruction.message}</strong>
                            ${instruction.street ? '<br><small>' + instruction.street + '</small>' : ''}
                        </div>
                        <div class="distance">
                            ${instruction.travelDistance ? DataFormatter.formatDistance(instruction.travelDistance) : ''}
                        </div>
                    </div>
                `;
            });
            ...
            instructionsHTML = `
                <div class="navigation-instruction">
                    <div class="icon">🏁</div>
                    <div class="text">
                        <strong>Siga a rota destacada no mapa</strong><br>
                        <small>Dirija-se da ${this.portaria.nome} até ${this.doca.nome}</small>
                    </div>
                </div>
            `;
        }
    ...
    startCountdown() {
        ...
        this.countdownTimer = setInterval(() => {
            ...
            if (timeRemaining <= 0) {
                countdownElement.innerHTML = '<span class="text-danger">⚠️ EXPIRADO</span>';
                ...
                if (timeRemaining < 5 * 60 * 1000) { // Menos de 5 minutos
                    countdownElement.innerHTML = `<span class="countdown-danger">⚠️ ${formatted}</span>`;
                } else if (timeRemaining < 15 * 60 * 1000) { // Menos de 15 minutos
                    countdownElement.innerHTML = `<span class="countdown-warning">⏰ ${formatted}</span>`;
                } else {
    ...
    addUserLocationMarker() {
        ...
        const userMarker = new atlas.data.Feature(new atlas.data.Point([
